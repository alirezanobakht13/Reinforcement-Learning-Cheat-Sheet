...
also policy could be a deterministic function
\begin{equation}
    \pi(s) = a
...
\section{Return}
The \emph{Return} is defined as:
\begin{equation}
G_t \doteq \sum_{k = 0}^H \gamma^kr_{t + k + 1}
\label{eq: total_reward}
\end{equation}
Where $\gamma$ is the \emph{discount factor} and $H$ is the \emph{horizon}, that can be infinite 
(of course, mathematically, it should be infinite. because otherwise policy will also depend on time).
...
\begin{equation}
        ...
        \text{where two latter equations can be written as one:}\\
...
to be able to use MDP as a model of a problem, the condition
\begin{equation}
    \begin{array}{l}
        Pr\{S_{t + 1}, R_{t + 1}|s_t,a_t,r_t,s_{t-1},a_{t-1},\dots,r_0,s_0,a_0\} =\\ Pr\{S_{t + 1}, R_{t + 1}|s_t,a_t\}
    \end{array}
...
Optimal policy is the policy that maximize value function (i.e. return) for each state, denoted by $ \pi^*$:
\begin{equation}
    \forall s,\pi \qquad V_{\pi^*}(s) \ge V_{\pi}(s)
\end{equation}
...
Forked from Francesco Saverio Zuppichini and improved by Alireza Nobakht

\scriptsize

\href{https://github.com/alirezanobakht13/Reinforcement-Learning-Cheat-Sheet}{https://github.com/alirezanobakht13/Reinforcement-Learning-Cheat-Sheet}
